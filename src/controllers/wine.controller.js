...
export const getWineById = (req, res) => {
  ...
  res.status(200).json({ message: "Wine found.", data: wine });
};
...
export const createWine = (req, res) => {
  ...
  res
    .status(201)
    .json({ message: "Whine created successfuolly.", date: newWine });
};
...
export const updateWine = (req, res) => {
  ...
  if (!name || !year) {
    return res
      .status(400)
      .json({ message: "PUT requires both name and year", data: null });
  }
